...
contract('CSetter', function (accounts) {
	...
	it('permissions check', async () => {
		...
		await expectRevert(collateral._setSafetyMarginSqrt(SAFETY_MARGIN_TEST, {from: user}), 'Smolmax: UNAUTHORIZED');
		await expectRevert(collateral._setLiquidationIncentive(LIQUIDATION_INCENTIVE_TEST, {from: user}), 'Smolmax: UNAUTHORIZED');
		await expectRevert(collateral._setLiquidationFee(LIQUIDATION_FEE_TEST, {from: user}), 'Smolmax: UNAUTHORIZED');
	});
	...
	it('safety margin boundaries', async () => {
		...
		await expectRevert(collateral._setSafetyMarginSqrt(failMin, {from: admin}), 'Smolmax: INVALID_SETTING');
		...
		await expectRevert(collateral._setSafetyMarginSqrt(failMax, {from: admin}), 'Smolmax: INVALID_SETTING');
	});
	...
	it('liquidation incentive boundaries', async () => {
		...
		await expectRevert(collateral._setLiquidationIncentive(failMin, {from: admin}), 'Smolmax: INVALID_SETTING');
		...
		await expectRevert(collateral._setLiquidationIncentive(failMax, {from: admin}), 'Smolmax: INVALID_SETTING');
	});
	...
	it('liquidation fee boundaries', async () => {
		...
		await expectRevert(collateral._setLiquidationFee(failMax, {from: admin}), 'Smolmax: INVALID_SETTING');
	});
});