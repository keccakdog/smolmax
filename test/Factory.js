...
contract('Factory', function (accounts) {
	...
	describe('create lending pool', () => {
		...
		it('revert if already exists', async () => {
			await expectRevert(factory.createCollateral(uniswapV2Pair1.address), "Smolmax: ALREADY_EXISTS");
			await expectRevert(factory.createBorrowable0(uniswapV2Pair2.address), "Smolmax: ALREADY_EXISTS");
			await expectRevert(factory.createBorrowable1(uniswapV2Pair3.address), "Smolmax: ALREADY_EXISTS");			
		});
		...
		it('initialize revert if not all three contracts are deployed', async () => {
			await expectRevert(factory.initializeLendingPool(uniswapV2Pair1.address), "Smolmax: BORROWABLE1_NOT_CREATED");
			await expectRevert(factory.initializeLendingPool(uniswapV2Pair2.address), "Smolmax: COLLATERALIZABLE_NOT_CREATED");
			await expectRevert(factory.initializeLendingPool(uniswapV2Pair3.address), "Smolmax: BORROWABLE0_NOT_CREATED");
		}); 
		...
		it('factory can only be set once', async () => {
			const lendingPool = await factory.getLendingPool(uniswapV2Pair1.address);
			await expectRevert((await Collateral.at(lendingPool.collateral))._setFactory(), "Smolmax: FACTORY_ALREADY_SET");
			await expectRevert((await Borrowable.at(lendingPool.borrowable0))._setFactory(), "Smolmax: FACTORY_ALREADY_SET");
			await expectRevert((await Borrowable.at(lendingPool.borrowable1))._setFactory(), "Smolmax: FACTORY_ALREADY_SET");
		}); 
		...
		it('revert if already initialized', async () => {
			await expectRevert(factory.initializeLendingPool(uniswapV2Pair1.address), "Smolmax: ALREADY_INITIALIZED");
			await expectRevert(factory.initializeLendingPool(uniswapV2Pair2.address), "Smolmax: ALREADY_INITIALIZED");
			await expectRevert(factory.initializeLendingPool(uniswapV2Pair3.address), "Smolmax: ALREADY_INITIALIZED");
		});
	});
	...
	describe('admin', () => {
		...
		it("change admin", async () => {
			await expectRevert(factory._setPendingAdmin(root, {from: root}), "Smolmax: UNAUTHORIZED");
			await expectRevert(factory._setPendingAdmin(root, {from: reservesAdmin}), "Smolmax: UNAUTHORIZED");
			await expectRevert(factory._acceptAdmin({from: root}), "Smolmax: UNAUTHORIZED");
		...
		it("change reserves admin", async () => {
			await expectRevert(factory._setReservesPendingAdmin(root, {from: root}), "Smolmax: UNAUTHORIZED");
			await expectRevert(factory._setReservesPendingAdmin(root, {from: admin}), "Smolmax: UNAUTHORIZED");
			await expectRevert(factory._acceptReservesAdmin({from: root}), "Smolmax: UNAUTHORIZED");
		...
		it("change reserves manager", async () => {
			await expectRevert(factory._setReservesManager(reservesManager, {from: reservesManager}), "Smolmax: UNAUTHORIZED");
			await expectRevert(factory._setReservesManager(reservesManager, {from: admin}), "Smolmax: UNAUTHORIZED");
